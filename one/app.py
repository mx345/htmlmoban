...
def reg1():
    if request.method == "GET":
        ...
        pwd = request.form.get("password3")
        pwd2 = request.form.get("password4")

        if not re.match("^[1][3,4,5,7,8][0-9]{9}$",uname):
        ...
        try:
            cur = db.cursor()
            cur.execute(
                "INSERT INTO sp_user VALUES (default, %s, md5(%s), sysdate(), sysdate(), '1', '1')", (uname, uname + pwd))
            cur.close()
            db.commit()
        except:
            abort(Response("用户注册失败！"))
